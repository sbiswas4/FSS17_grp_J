...
print("\n")
print("{:15} {:15} {:8}".format("LEAF 1", "LEAF 2", "CONTRAST"))
#print("\n")
for p in range(len(leaves)):
    for q in range((p+1),len(leaves)):
        if leaves[p]['avg_dom'] > leaves[q]['avg_dom']:
            print("{:15} {:15} {:8}".format(leaves[p]['x'],leaves[q]['x'],"Plan"))
        elif leaves[p]['avg_dom'] < leaves[q]['avg_dom']:
            print("{:15} {:15} {:8}".format(leaves[p]['x'], leaves[q]['x'], "Monitor"))
